...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = {
      val allWays: List[(Block,Move)] = List((this.up, Up),(this.down, Down),(this.left,Left),(this.right,Right))
      allWays.filter {
        case (blockNext, _) => terrain(blockNext.b1) && terrain(blockNext.b2)
      }
    }
    ...
    def legalNeighbors: List[(Block, Move)] = {
      val legalWays = neighbors
        legalWays.filter{
        case (blockFilter,_) => blockFilter.isLegal
      }
    }
